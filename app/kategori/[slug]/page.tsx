...
export default function CategoryPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <Section><div className="text-[13px] leading-relaxed space-y-3" style={{ color: 'var(--ink2)' }}>{cat.longDescription.split('\n\n').map((p, i) => <p key={i}>{p}</p>)}</div></Section>
      <Section title={`${parks.length} ${cat.name.toLowerCase()}`}>{parks.length > 0 ? <ParkGrid>{parks.map(p => <ParkCard key={p.id} name={p.name} href={`/park/${p.slug}`} description={p.shortDescription} city={p.city} county={p.county} category={p.category} featured={p.featured} audience={p.audience} />)}</ParkGrid> : <p className="text-[13px]" style={{ color: 'var(--ink3)' }}>Ingen oppføringer ennå.</p>}</Section>
