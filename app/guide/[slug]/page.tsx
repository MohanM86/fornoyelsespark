...
export default function GuidePage({ params }: Props) {
  ...
  return (
    <>
      ...
      <h1 className="text-xl font-bold mb-2" style={{ color: 'var(--txt)' }}>{guide.title}</h1>
      <p className="text-[14px] leading-relaxed mb-6" style={{ color: 'var(--txt-s)' }}>{guide.intro}</p>

      {/* TOC */}
      <div className="rounded-xl p-4 mb-6" style={{ background: 'var(--card)', border: '1px solid var(--brd)' }}>
        <p className="text-[11px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--txt-m)' }}>Innhold</p>
        {guide.sections.map((s, i) => (
          <a key={i} href={`#s${i}`} className="block text-[13px] py-1 hover:text-white transition-colors" style={{ color: 'var(--txt-s)' }}>{s.heading}</a>
        ))}
      </div>
      ...
      <FAQ items={guide.faq} />
      <div className="mt-8"><ChatInput placeholder="Spør om fornøyelsesparker..." /></div>
    </>
  )
}
